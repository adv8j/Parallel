							...
							SymbolTable* global)
{
	...
	{
		std::string message = "Struct " + struct_info->name + " expects " +
							  std::to_string(struct_info->members.size()) + " members but got " +
	...
	for(int i = 0; (long unsigned int)i < node->children.size(); i++)
	{
		...
		{
			std::string message = "Struct " + struct_info->name + " expects member " +
		...
		{
			std::string message = "Struct " + struct_info->name + " expects member " +
								  std::to_string(i) + " of type " + member_type.to_string() +
								  " but got " + child_type.to_string();
			yy_sem_error(message);
			return false;
		}
		...
		{
			std::string message = "Struct " + struct_info->name + " expects member " +
								  std::to_string(i) + " of type " + member_type.name + " but got " +
								  child_type.name;
			yy_sem_error(message);
			return false;
		}
